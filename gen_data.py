...
from envs.wrappers.record import RecordEpisode
from utils.array import to_tensor, recursive_slice
...
from agents.ppo import Agent as PPOAgent
from agents.sac.agent import Agent as SACAgent
...
def eval(
    ...
    env_cfg = EnvConfig(
        ...
        cat_state=True,
        cat_pixels=False,
        task_plan_fp=f"task_plans/{task}/{subtask}/{split}/{obj_name}.json",
        spawn_data_fp=(
            ASSET_DIR
        ...
        wrappers = [
            partial(
                ...
                valid_episode_labels=(
                    [
                        "straightforward_success",
                    ]
                    if subtask == "pick"
                    else (
                        ["placed_in_goal_success"]
                        if subtask == "place"
                        else (
                            ["open_success"]
                            if subtask == "open"
                            else (
                                ["closed_success", "success_then_excessive_collisions"]
                                if subtask == "close" and obj_name == "fridge"
                                else ["closed_success"]
                            )
                        )
                    )
    ...
    def check_done():
        if SAVE_TRAJECTORIES:
            # NOTE (anonymous): eval_envs.env._env is bad, fix in wrappers instead (prob with get_attr func)
