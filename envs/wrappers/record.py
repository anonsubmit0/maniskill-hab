...
from envs.sequential_task import SequentialTaskEnv
from envs.pick import PickSubtaskTrainEnv
from envs.place import PlaceSubtaskTrainEnv
from envs.open import OpenSubtaskTrainEnv
from envs.close import CloseSubtaskTrainEnv
from utils.io import NoIndent, NoIndentSupportingJSONEncoder
from utils.misc import put_info_on_image
from utils.label_dataset import get_episode_label_and_events
        ...
        **kwargs,
        ...
        if self.save_trajectory:
            ...
            if self._trajectory_buffer is None:
                ...
                # NOTE (anonymous): actually, here we save *before* all wrappers since we want to have the dataset
    ...
    def flush_trajectory(
        ...
        for env_idx in env_idxs_to_flush:
            ...
            if save:
                ...
                if self.label_episode:
                    episode_label, episode_events, episode_events_verbose = (
                        get_episode_label_and_events(
                            ...
                            # NOTE (anonymous): we disclude reset infos for episode labeling
                            common.index_dict_array(
                                self._trajectory_buffer.info,
                                (
                                    slice(start_ptr + 1, end_ptr),
                ...
                # NOTE (anonymous): we don't save info for now
                # # NOTE (anonymous): infos also need special processing
