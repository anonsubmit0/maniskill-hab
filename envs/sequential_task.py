...
from mani_skill.utils.structs import Actor, Pose, Link, Articulation
...
class SequentialTaskEnv(SceneManipulationEnv):
    ...
    def process_task_plan(self, sampled_subtask_lists: List[List[Subtask]]):
        ...
        for subtask_num, parallel_subtasks in enumerate(zip(*sampled_subtask_lists)):
            ...
            if isinstance(subtask0, PickSubtask):
                ...
                # NOTE (anonymous): this is a bit tricky, since prepare_groceries sometimes has an articulation config
                ...
                if all(
                    [
                        subtask.articulation_config is not None
                ...
                self.task_plan.append(
                    PickSubtask(
                        ...
                        # NOTE (anonymous): pick subtask might have different kinds of articulations
                ...
                # NOTE (anonymous): see notes above regarding merged articulation config
                if all(
                    [
                        subtask.articulation_config is not None
                        for subtask in parallel_subtasks
                ...
                parallel_subtasks: List[OpenSubtask]

                # NOTE (anonymous): current MS3 requires all parallel articulations be the same
                assert all_equal(
                    [subtask.articulation_type for subtask in parallel_subtasks]
                )
                assert all_equal(
                    [
                ...
                parallel_subtasks: List[CloseSubtask]
                ...
                # NOTE (anonymous): currently a band-aid solution, teleport bowl inside kitchen drawer away
    ...
    def _create_merged_articulation_from_articulation_ids(
        ...
        for env_num, aid in enumerate(articulation_ids):
            ms_articulation = self.scene_builder.articulations[f"env-{env_num}_{aid}"]
            scene_idx_to_physx_articulation_objs[env_num] = ms_articulation._objs[
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options):
        with torch.device(self.device):
            ...
            if self.task_plan_idxs is None:
                low = torch.zeros(len(self.build_config_idxs), dtype=torch.int)
    ...
    def get_state_dict(self):
        ...
        state_dict["task_plan_idxs"] = copy.deepcopy(self.task_plan_idxs)
    ...
    def set_state_dict(self, state_dict: Dict):
        ...
        task_plan_idxs = state_dict.get("task_plan_idxs")
    ...
    def evaluate(self):
        ...
        # NOTE (anonymous): update ee_rest_world_pose every step since robot moves
    ...
    def _progressive_task_check_success(self):
        ...
        # NOTE (anonymous): while Habitat also checked things like holding vs not,
    ...
    def _navigate_check_success(
        ...
        navigated_close = (
            torch.norm(
                goal.pose.p[env_idx, :2] - self.agent.robot.pose.p[env_idx, :2], dim=1
            )
            <= self.navigate_cfg.navigated_sucessfully_dist
        )
    ...
    # NOTE (anonymous): for now, define keys that will always be added to obs. leave it to
    ...
    # REWARD (Ignored here)
    # -------------------------------------------------------------------------------------------------
    # NOTE (anonymous): this env does not have dense rewards since rewards are used for training subtasks.
    ...
    # -------------------------------------------------------------------------------------------------
    # NOTE (anonymous): also included the old "cameras" mode from MS2 since HAB renders this way
    # -------------------------------------------------------------------------------------------------
