...
if [ "$TASK" = "set_table" ] && [ "$SUBTASK" = "open" ] && [ "$OBJ" = "kitchen_counter" ]; then
    ...
    extra_args="$stem=\"True\" eval_$stem=\"False\""
else
    extra_args=""
fi
...
if [ -f "$resume_config" ] && [ -f "$resume_logdir/models/latest.pt" ]; then
    ...
    SAPIEN_NO_DISPLAY=1 python train_ppo.py "$resume_config" resume_logdir="$resume_logdir" \
    ...
    SAPIEN_NO_DISPLAY=1 python train_ppo.py configs/ppo_pick.yml \
