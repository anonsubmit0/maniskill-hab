...
from utils.misc import all_equal

from envs.planner import (
    PlanData,
    TaskPlan,
    PickSubtask,
    PlaceSubtask,
...
class GenTaskPlanArgs:
    ...
    def __post_init__(self):
        ...
        assert self.subtask in ["pick", "place", "open", "close", "sequential"]
    ...
    for init_config_name in tqdm(scene_builder._rearrange_configs):
        ...
        for actor_id, (base_articulation_id, link_num) in zip(
            ...
            if args.task == "set_table" or (
                ...
                # NOTE (anonymous): For RCAD kitchen_counter and fridge, the above URDF object has one less joint than links.
                #       SAPIEN left-appends an extra `''` joint so the links and joints correspond, so we follow suit
                # NOTE (anonymous): not sure if this holds true for *all* articulations, or just these specific ones. mileage may vary
