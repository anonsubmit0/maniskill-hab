...
from agents.sac.agent import Agent
from agents.sac.replay import PixelStateBatchReplayBuffer
from envs.make_env import make_env, EnvConfig
from utils.array import to_tensor, to_numpy
...
def store_batch(
    ...
    # NOTE (anonymous): requre continuous task bc of replay buffer
    if trunc.any():
        return
...
def train(cfg: TrainConfig):
    ...
    # MEMORY
    # -------------------------------------------------------------------------------------------------
    # NOTE (anonymous): we use horizon - 1 since we ignore the data
