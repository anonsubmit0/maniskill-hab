...
from utils.array import recursive_slice
from agents.ppo import Agent, DictArray
...
def train(cfg: TrainConfig):
    ...
    # MEMORY
    # -------------------------------------------------------------------------------------------------
    # NOTE (anonymous): we never "reset" this, just overwrite old data
    # -------------------------------------------------------------------------------------------------
    obs = DictArray(
    ...
    for iteration in range(cfg.algo.num_iterations):
        ...
        if eval_envs is not None and check_freq(cfg.algo.eval_freq):
            eval_envs.reset()
            while len(eval_envs.success_once_queue) < cfg.algo.eval_episodes:
                with torch.no_grad():
