...
from .interact_scene_builder import ReplicaCADInteractSceneBuilder

MS_SIM_CONFIG = dict(sim_freq=100, control_freq=20)


def make_habitat_env(
    config_yaml_name="hab-scene-manip", concur_render=True, auto_sleep=True
):
    import habitat_sim  # type: ignore
    import habitat  # type: ignore
    from habitat_baselines.common.habitat_env_factory import (  # type: ignore
...
def make_env() -> Tuple[gym.vector.VectorEnv, bool]:
    if "Habitat" in args.bench_preset:
        config_yaml_name = (
            "interact" if "Interact" in args.bench_preset else "hab-scene-manip"
        )
...
def interact_bench():
    ...
    preloaded_actions_fp = (
        ASSET_DIR
        / "scene_datasets/replica_cad_dataset/hab2_bench_assets/interact_bench_actions.txt"
    )
